...
CONFIRMED:     confirmed=True → po_inventory wiped and reloaded from source.
NOT_CONFIRMED: confirmed=False → no wipe, no source fetch, repo unchanged.
"""
...
def test_refresh_confirmed() -> None:
    """confirmed=True → old items wiped, source fetched, new items loaded; counts logged."""
    repo = FakeRepository()
    ...
    assert repo.count_po_items() == 1
    assert repo.get_purchase_order("PO-OLD") == []  # old items gone
    assert len(repo.get_purchase_order("PO-NEW")) == 1  # new items loaded
...
def test_refresh_not_confirmed() -> None:
    ...
    refresh_all(_FailSource(), repo, confirmed=False)  # must not raise

    assert repo.count_po_items() == 1  # unchanged
    assert len(repo.get_purchase_order("PO-OLD")) == 1  # original row still present
